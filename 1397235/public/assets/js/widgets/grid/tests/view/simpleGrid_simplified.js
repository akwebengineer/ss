/**
 * A view that uses a configuration object to render a grid widget that shows a simplified view of the grid
 *
 * @module GridView
...
define([
    ...
    'widgets/grid/tests/view/updateView',
    'text!widgets/grid/tests/templates/simplifiedGridContainers.html',
    'widgets/grid/tests/dataSample/rbac',
    'mockjax'
], function(Backbone, GridWidget, configurationSample, ZonePoliciesAddView, GridOverlayView, ZonePoliciesModel, firewallPoliciesData, QuickView, UpdateView, SimplifiedGrid, rbacData, mockjax){
...
//            this.render();
        ...
        updateRow: function (layoutOptions, rowData, success) {
            var self = this,
                saveRow = function (newRow) {
                    var updatedRow = !_.isUndefined(rowData.defaultRow) ? _.extend(rowData.defaultRow, newRow) : newRow;
                    success(updatedRow);
                };
            if (_.isUndefined(this.updateView)){
                this.updateView = new UpdateView({
                    "el": '.update-row-wrapper',
                    "layoutOptions": layoutOptions,
                    "cancelRow": _.bind(this.closeUpdateRow, self),
                    "updateRightPanel": _.bind(this.options.updateRightPanel, self)
                });
            }
            this.updateView.render(rowData.defaultRow || rowData.originalRow, _.bind(saveRow, self));
            this.openUpdateRow();
        },

        editRow: function (selectedRows) {
            var self = this,
                saveRow = function (newRow) {
                    self.gridWidget.editRow(selectedRows.rowOnHover.rowData, newRow);
                };
            if (_.isUndefined(this.updateView)){
                this.updateView = new UpdateView({
                    "el": '.update-row-wrapper',
                    "layoutOptions": selectedRows.rowOnHover.gridHeaderLayout,
                    "cancelRow": _.bind(this.closeUpdateRow, self)
                });
            }
            this.updateView.render(selectedRows.rowOnHover.rowData,  _.bind(saveRow, self));
            this.openUpdateRow();
        },

        openUpdateRow: function () {
            this.updateWrapperContainer.show();
            this.gridWidget.toggleGridHeader();
            this.options.toggleMainPanel(false);
            this.options.updateRightPanel();
        },

        closeUpdateRow: function () {
            this.options.destroyRightPanel();
            this.options.toggleMainPanel(true);
            this.gridWidget.toggleGridHeader();
        },
        ...
        render: function () {
            ...
            configurationSample.simpleGrid_simplified.createRow = {
                "onCreate": _.bind(self.updateRow, self)
            };

            configurationSample.simpleGrid_simplified.editRow = {
                "onEdit": _.bind(self.updateRow, self)
            };
...
//                search: this.search,
        ...
        bindGridEvents: function () {
            var self = this;
            this.$el
                .bind(this.actionEvents.createEvent.name, function(e, addGridRow){ //called if onCreate callback is not defined
                    console.log(addGridRow);
//                    self.addRowOnOverlay(addGridRow); //add row on an overlay
                })
                .bind(this.actionEvents.updateEvent, function(e, updatedGridRow){
                    console.log(updatedGridRow);
//                    self.updateRowOnOverlay(updatedGridRow);//update row on an overlay
                })
...
//                    console.log(selectedRows);
                ...
                .bind(this.actionEvents.testCloneHover, function(e, selectedRows){
                    console.log("Test Clone on Row Hover in the Grid");
                    console.log(selectedRows);
                    console.log(self.gridWidget.getGridHeaderLayout());
                })
                .bind(this.actionEvents.testInfoHover, function(e, selectedRows){
                    console.log("Test Info on Row Hover in the Grid");
                    console.log(selectedRows);
                })
...
//                    console.log(selectedRows);
            ...
            this.$el.find(".grid-widget")//binds events to the current simplified view grid only since the container contains more than one grid
                .bind("slipstreamGrid.resized:gridTable", function(e, options){
                    console.log("gridOnGridWidthResize");
                    if (!_.isUndefined(self.updateView)) {
                        self.updateView.adjustStyle(options.gridHeaderLayout, true);
                    }
                });
        },

        addRowOnOverlay: function(addGridRow) {
            var self = this;
        ...
        updateRowOnOverlay: function(updateGridRow) {
            var self = this;
            var view = new ZonePoliciesAddView({
...
//            console.log(cellData);
        ...
        addContainers: function () {
            this.$el.append(SimplifiedGrid);
            this.updateWrapperContainer = this.$el.find('.update-row-wrapper');
