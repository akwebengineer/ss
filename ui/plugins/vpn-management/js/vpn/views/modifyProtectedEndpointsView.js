 ...
 *
 * @module modifyProtectedEndpointsView
    ...
    var ProtectedEndpointsView = Backbone.View.extend({
        ...
        addListBuilderProtectedInterface: function(id, selectedDevices) {
            ...
            this.interfaceListBuilder.build(function() {
                ...
                var routingType = self.options.vpnData["routing-type"];
                ...
                if ((cellData[0] === "") || (self.protectedCellType() !== "ADDRESS")) {
                    ...
                    if (map) {
                        ...
                        if (lastDataSaved) {
                            ...
                            // if no hash table take the data from the original row data
                            // because the formatter can't write id's do not have to strip the "Address: " off the cell data.
                            lastDataSaved = self.passedRowData.originalRowData['protected-networks']['protected-network'];
                            // build array of ids
                            lastDataSaved.forEach (function(object) {
                                idList.push(object.id);
                            });
                        };
                    } else {
                    ...
                    // Because ids come from model instead of cellData string
                    // there is no protected type to remove from the list of ids.
        ...
        addListBuilderProtectedZone: function(id, selectedDevices) {
            ...
            var listContainer = this.$el.find('#' + id),
                conf = {
                    ...
                    // device-id of the grid row clicked on for modify comes from moid
                    urlParameters: {"device-id": (this.passedRowData.originalRowData["device-moid"].split(":"))[1], "ui-session-id":  this.uuid}
                };
            ...
            // the last list builder built.  Hide the zone and interface list builder's "Loading..." message
            // Only allow the address list builder to be displayed.
        ...
        getProtectedListBuilders: function(e){
            ...
            var getSelectedAddressCallback = function(response){
                ...
                // protected networks request body is array of objects
        ...
        // this overlay. By looking at the first field values for zone, interface and address columns,
        ...
        protectedCellType: function () {
            ...
            // If we have saved some protected resource to model then get from model
            if (map) {
            ...
            // Modify on first time a row is edited, will read from the original row data instead of
            // reading from the model hash table.  We add to the hash table after a row has been
            // edited so that we can save the edits outside of the grid.  There is no place in
            // the grid to save the address id objects.
        ...
        render: function(){
            ...
            var formConfiguration = new ProtectedEndpointsFormConfiguration(this.context);
