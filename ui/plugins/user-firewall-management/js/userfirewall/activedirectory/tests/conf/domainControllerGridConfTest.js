/**
 * UT for Domain Controller Grid Configuration
 *
 * @module domainControllerGridConfTest
...
define([
    ...
    describe('Domain Controller  Grid Configuration UT', function () {
        ...
        it('Checks if the Domain Controller Grid Configuration object is created properly', function () {
            conf.should.exist;
        });

        it('Checks table defaults', function () {
            values['tableId'].should.be.equal('domain_controllers_grid');
            values['numberOfRows'].should.be.equal(10);
            values['jsonRoot'].should.be.equal(Constants.ACTIVE_DIRECTORY.DOMAIN_CONTROLLER.GRID_JSON_ROOT);
        ...
        it('Checks the Json records', function () {
            ...
            result = values['jsonRecords']({records: 1, 'domain-controllers': {'total': total}});
        ...
        describe('Check table columns', function () {
            ...
            it('Checks if the domain controller name column is defined', function () {
                var col;
                _.each(columns, function (eachCol) {
                    if (eachCol.name === 'domain-controller-name') {
                        col = eachCol;
                    }
                });
                _.isUndefined(col).should.be.equal(false);
                col.index.should.be.equal('domain-controller-name');
                col.sortable.should.be.equal(false);

                col.editCell.type.should.be.equal('input');
                col.editCell.pattern.should.be.equal('^[a-zA-Z0-9][a-zA-Z0-9-_.:\/]{0,62}$');
                col.editCell.required.should.be.equal(true);
            });

            it('Checks if the domain-controller ip address column if defined', function () {
                var col;
                _.each(columns, function (eachCol) {
                    if (eachCol.name === 'domain-controller-ip-address') {
                        col = eachCol;
                    }
                });

                _.isUndefined(col).should.be.equal(false);
                col.index.should.be.equal('domain-controller-ip-address');
                col.sortable.should.be.equal(false);
